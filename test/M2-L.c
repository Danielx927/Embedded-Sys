/*
 * Test ID: M2-L
 * Description: Execute a 90° left turn with IMU-stabilised differential drive.
 * This test verifies 90° left turn using IMU heading control, differential motor speeds, and speed recovery post-turn.
...
int main() {
    ...
    printf("M2-L Test: 90° Left Turn with IMU - Starting\n");
    ...
    // Issue LEFT_TURN: Start turn with reduced speed (10 RPM equiv ≈0.031 m/s average)
    ...
    while (millis_now() - start_time < 10000) {  // 10s max
        ...
        if (turning) {
            // Heading PID for left turn: Adjust differential (left slower, right faster)
            ...
            // For left turn, positive adjust means more right speed
            float left_duty = base_duty - 0.15f * heading_adjust;  // Reduce left
            float right_duty = base_duty + 0.15f * heading_adjust; // Increase right
            ...
            // Check settle: Within ±1 RPM (0.005 m/s) for ≥1s, left recovers to 20 RPM ≤0.5s
            float left_error = fabsf(left_speed - recovery_speed);
            if (left_error <= 0.005f && (now - settle_start >= 1000)) {
                settled = true;
                printf("Left wheel settled at %.3f m/s (±0.005) for 1s\n", left_speed);
                break;
            }
        }
    ...
    bool left_recovery_ok = (fabsf(left_speed - recovery_speed) <= 0.005f);

    // No faults: Assume no timeouts if completed
    bool no_faults = settled;

    // Pass/Fail
    bool pass = time_ok && overshoot_ok && settle_ok && left_recovery_ok && no_faults;
    if (pass) {
        printf("PASS: Reach ≤5s; overshoot ≤15°; settle ≥1.0s; Left recovers to 20±1 RPM ≤0.5s; No faults.\n");
    } else {
        ...
        if (!left_recovery_ok) printf("- Left recovery: %.3f m/s (target %.3f ±0.005)\n", left_speed, recovery_speed);
    ...
    printf("- Final right speed: %.3f m/s\n", right_speed);
